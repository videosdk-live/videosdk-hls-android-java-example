...
public class ViewerFragment extends Fragment implements View.OnClickListener, StyledPlayerView.ControllerVisibilityListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            ...
            trackSelectionParameters = new TrackSelectionParameters.Builder(/* context= */ mContext).build();
            clearStartPosition();
        }
        ...
        if (networkUtils.isNetworkAvailable()) {
            networkUtils.getToken(new ResponseListener<String>() {
                ...
                public void onResponse(String token) {
                    networkUtils.checkActiveHls(token, meeting.getMeetingId(), url -> {
                        downStreamUrl = url;
    ...
    private void ActionListener(View view) {
        ...
        btnReactions.setOnClickListener(v -> reactionsLayout.setVisibility(View.VISIBLE));

        view.findViewById(R.id.loveEyes).setOnClickListener(v -> {
            showEmoji(getResources().getDrawable(R.drawable.love_eyes_emoji));
            meeting.pubSub.publish("emoji", "love_eyes", pubSubPublishOptions);
            reactionsLayout.setVisibility(View.GONE);
        });

        view.findViewById(R.id.laughing).setOnClickListener(v -> {
            meeting.pubSub.publish("emoji", "laughing", pubSubPublishOptions);
            reactionsLayout.setVisibility(View.GONE);
        });

        view.findViewById(R.id.thumbs_up).setOnClickListener(v -> {
            meeting.pubSub.publish("emoji", "thumbs_up", pubSubPublishOptions);
            reactionsLayout.setVisibility(View.GONE);
        });

        view.findViewById(R.id.celebration).setOnClickListener(v -> {
            meeting.pubSub.publish("emoji", "celebration", pubSubPublishOptions);
            reactionsLayout.setVisibility(View.GONE);
        });

        view.findViewById(R.id.clap).setOnClickListener(v -> {
            meeting.pubSub.publish("emoji", "clap", pubSubPublishOptions);
            reactionsLayout.setVisibility(View.GONE);
        });

        view.findViewById(R.id.heart).setOnClickListener(v -> {
    ...
    public void onVisibilityChanged(int visibility) {
        ...
        if (reactionsLayout.getVisibility() == View.VISIBLE)
                ...
                for (Tracks.Group trackGroup : tracks.getGroups()) {
                    ...
                    if (trackGroup.getMediaTrackGroup().type == 2) {
                        for (int i = 0; i < trackGroup.length; i++) {
                            ...
                            if (isSelected)
                                btnQuality.setText(String.valueOf(trackFormat.width).concat("p"));
                        }
                    }
                }
            }
    ...
    public void showProducts() {
        ...
        View v3 = LayoutInflater.from(mActivity).inflate(R.layout.products_layout, null);
